...
# Count repeated measures per fish
fish_repeats <- data %>%
  group_by(ID) %>%
  summarise(
...
SGR_diagnostic <- lm(SGR_2m ~ mass*factor(Temp), data = data_sgr)
...
data_sgr$SGR_2m_resid <- resid(lmer(SGR_2m ~ mass + (1|ID), data_sgr)) #residuals of SGR are also a good option for more effectively mass-standardizing my growth measurements. 
