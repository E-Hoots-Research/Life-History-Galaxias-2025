...
```{r, echo = F}
figS1 <- ggplot(data = data[which(data$Temp == 18 | data$Temp == 23),], aes(x = mass, y = lead(SGR_2m), colour = factor(Temp), fill = factor(Temp))) +
  geom_point(size = 2) +
...
figS2 <- ggplot(data = data[which(data$Temp == 18 | data$Temp == 23),], aes(x = mass, y = SGR, colour = factor(Temp), fill = factor(Temp))) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = T) +
  scale_color_manual(values =c("skyblue", "orangered"),
    guide = guide_legend(override.aes = list(size = 4))) +
...
figS2 #mass scaling of 1-month SGR (using current mass as M1 in SGR calc — growth of following month)


figS3 <- ggplot(data = data[which(data$Temp == 18 | data$Temp == 23),], aes(x = mass, y = lead(SGR), colour = factor(Temp), fill = factor(Temp))) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = T) +
  scale_color_manual(values =c("skyblue", "orangered"),
    guide = guide_legend(override.aes = list(size = 4))) +
...
figS3  #mass scaling of 1-month SGR (using current mass as M2 in SGR calc — growth of month prior)


```

With only a visual inspection, it looks like SGR is already pretty well standardized to the intermediate mass measurement when SGR is taken from a two month interval. Perhaps trending a bit too negatively for the 18C treatment, but let's give it a go as is.

Now we move on to look at the relationship between SGR and mass-standardized SMR.

```{r, echo = F}

Fig1 <- ggplot(data[which(data$ds == "B1"),], aes(x = lead(SGR_2m), y = SMR_mass, colour = factor(Temp), fill = factor(Temp),  group = factor(Temp))) + #shape = factor(Month),
  geom_point(size = 2) +
...
Fig2 # MMR vs. SGR (2 months)

# data wrangling for model purposes 
...
plot(lmod_SMR) #looks okay, maybe a slight quadratic trend that should be watched out for
summary(lmod_SMR)

lmod_MMR <- lmer(log(MMR_mass) ~ log(Ratio) +(1 | ID), data = data_matstat_18)
plot(lmod_MMR) #positive linear trend in residuals vs. fitted values, no homoscedascity, not suitable for linear regression analysis

#try again without random effects term (simple linear model):

lmod_MMR1 <- lm(log(MMR_mass) ~ log(Ratio), data = data_matstat_18)
plot(lmod_MMR1) #this fits much better, although now does not account for repeated measures on individuals. 
summary(lmod_MMR1)

#SMR vs. gut ratio as a simple linear model:
lmod_SMR1 <- lm(log(SMR_mass) ~ log(Ratio), data = data_matstat_18)
plot(lmod_SMR1)
summary(lmod_SMR1)
